                  ...
                  hid_t type_id, hid_t space_id, hid_t dcpl_id, hid_t dapl_id, hid_t dxpl_id, void **req)
{
    ...
    if (CURLE_OK != curl_easy_setopt(curl, CURLOPT_POST, 1))
        FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTSET, NULL, "can't set up cURL to make HTTP POST request: %s",
                        curl_err_buf);
    printf("h5_dataset_create_request_data: %s\n", create_request_body);
...
#endif
    printf("h5_dataset_create_repsonse_data: %s\n", response_buffer.buffer);
                ...
                hid_t dxpl_id, void **req)
{
    ...
    /* Locate dataset and set domain 发送curl请求，获取dataset请求 */
    ...
    // printf("response buf (dataset_open): %s\n", response_buffer.buffer);
...
void
processInput(const char *input, Vector *result1, Vector *result2)
{
    char *inputCopy = strdup(input); // Make a copy of the input string
    char *token;
    char *saveptr1;
    token = strtok_r(inputCopy, ",", &saveptr1);
    while (token != NULL) {
        char *subtoken;
        char *saveptr2;
        int   count = 0;
        ...
        while (subtoken != NULL) {
            if (count == 0) {
                value1 = atoi(subtoken);
            }
            else if (count == 1) {
                value2 = atoi(subtoken);
            }
            count++;
            subtoken = strtok_r(NULL, ":", &saveptr2);
        }
        pushBack(result1, value1);
        pushBack(result2, value2);
...
char *
vector_to_string(Vector *vector, Vector *offset)
{
    // 预估结果字符串所需的总长度
    size_t total_length = 0;
    for (size_t i = 0; i < vector->size; i++) {
        // 计算当前维度的字符串长度
        size_t dim_length = snprintf(NULL, 0, "[%d:%d:1]", offset->data[i], vector->data[i]);
        total_length += dim_length;
    }

    // 加上逗号和结尾的空字符
    total_length += vector->size - 1 + 1;
    ...
    // 构建结果字符串
    for (size_t i = 0; i < vector->size; i++) {
        char buffer[20]; // 假设20足够存放每个数字和分隔符

        // 将当前维度的值转换为字符串
        sprintf(buffer, "%d:%d:1", offset->data[i], vector->data[i]);
...
void
splitRangeAndGetInfo(int j, int n, int sum, size_t *Node_Size, Vector *count, Vector *offset,
                     char **select_str)
{
    int rangeSize         = count->data[j] - offset->data[j]; // size
    int elementsPerPart   = rangeSize / n;
    int remainingElements = rangeSize % n;
    // printf("-> | split index:%d , size: %d |\n", j, n);
    int currentStart = offset->data[j];
    int currentEnd   = offset->data[j];
    // printf("-> | split count:%d , offset: %d |\n", count->data[j], offset->data[j]);
    for (int i = 0; i < n; i++) {
        // printf("-> | 1 |\n");
        int elementsInThisPart = elementsPerPart;
        if (i < remainingElements) {
            elementsInThisPart++;
        }
        currentEnd = currentStart + (elementsInThisPart);
        // printf("-> | 2 |\n");

        count->data[j]  = currentEnd;
        offset->data[j] = currentStart;
        Node_Size[i]    = (currentEnd - currentStart) * sum;
        // printf("-> | NodeSize:%d |\n", Node_Size[i]);
        char *tmp = vector_to_string(count, offset);
        // printf("-> | 3 tmp: %s |\n", tmp);
        select_str[i] = strdup(tmp); // 复制字符串内容
        // printf("NO%d :  %s\n", i, tmp);
...
size_t write_header_callback(void *contents, size_t size, size_t nmemb, void *userp) {
    ...
    dataset_transfer_info *dataset_info = (dataset_transfer_info *)userp;

    // 查找 Content-Length 字段
    if (strncasecmp(header, "Content-Length:", 15) == 0) {
        // 提取 Content-Length 的值
        char *lengthStr = header + 15;  // 跳过 "Content-Length:"
        
        // 去除可能的空格和换行符
        while (*lengthStr == ' ' || *lengthStr == '\t' || *lengthStr == '\r' || *lengthStr == '\n') {
            lengthStr++;
        }

        // 将 Content-Length 转换为 size_t 类型并存储
        dataset_info->compression_length = (size_t)atoll(lengthStr);  // 使用 atoll 将字符串转换为 size_t

        // 返回 totalSize，表示回调已成功处理该数据
        return totalSize;
    }
    else if (strncmp(header, "Content-Encoding:", 17) == 0) {
        strncpy(dataset_info->compression_type, header + 17, sizeof(dataset_info->compression_type) - 1);
        // 去除末尾的换行符和空格
        dataset_info->compression_type[strcspn(dataset_info->compression_type, "\r\n")] = '\0';
    }
                ...
                hid_t _file_space_id[], hid_t dxpl_id, void *buf[], void **req)
{
    ...
    char *Server_Num        = getenv("SERVER_NUM");
    char *XrdMultiServerUrl = getenv("XRDMULTI_ENDPOINT");
    if (Server_Num == NULL || XrdMultiServerUrl == NULL) {
        FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                        "Env SERVER_NUM or XRDMULTI_ENDPOINT can't be empty !");
    }
    // printf("Env URLs:%s\n", XrdMultiServerUrl);

    int ServerNodeNum = atoi(Server_Num);
    if (ServerNodeNum <= 0 || ServerNodeNum > 10) {
        FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL, "SERVER_NUM error");
    }

    char *URLs_by_Myself[ServerNodeNum];
    char *url_string_copy = strdup(XrdMultiServerUrl);
    if (!url_string_copy) {
        FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL, "Memory allocation failed");
    }

    char *current_url = strtok(url_string_copy, "{},");
    int index = 0;
    while (current_url != NULL && index < ServerNodeNum) {
        while (*current_url == ' ') {
            current_url++;
        }

        URLs_by_Myself[index] = strdup(current_url);
        if (!URLs_by_Myself[index]) {
            FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL, "Memory allocation failed");
        }

        current_url = strtok(NULL, "{},");
        index++;
    }

    // printf("URL array:\n");
    // for (int i = 0; i < ServerNodeNum; ++i) {
    //     printf("%s\n", URLs_by_Myself[i]);
    // }
    //*************************end*****************************
    ...
    size_t                 host_header_len     = 0;
    int                    url_len             = 0;
    CURLM                 *curl_multi_handle[ServerNodeNum];
    dataset_transfer_info *transfer_info = NULL;
    size_t                *node_size     = (size_t *)RV_malloc(ServerNodeNum * sizeof(size_t));
    if (node_size == NULL) {
        // 错误处理：内存分配失败
        FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL, "can't allocate space for node_size");
    }
    memset(node_size, 0, ServerNodeNum * sizeof(size_t));
    

    char **Select_Node = (char **)RV_malloc(ServerNodeNum * sizeof(char *));
    // create n Vector struct numpy
    bool   flag_Node   = false;
    // char
    // *URLs_by_Myself[]={"https://hepslustretest02.ihep.ac.cn:1094","https://hepslustretest02.ihep.ac.cn:1093","https://hepslustretest02.ihep.ac.cn:1092"};

    if ((transfer_info = RV_calloc(count * ServerNodeNum * sizeof(dataset_transfer_info))) == NULL)
    ...
    for (int i = 0; i < ServerNodeNum; i++)
        curl_multi_handle[i] = curl_multi_init();

    /* Initialize arrays and check arguments */
    for (size_t i = 0; i < count * ServerNodeNum; i++) {

        int z = i / ServerNodeNum;
        if (!buf[z])
        ...
        transfer_info[i].Node_Size        = (size_t *)RV_malloc(ServerNodeNum * sizeof(size_t));
        ...
        if (CURLE_OK != curl_easy_setopt(transfer_info[i].curl_easy_handle, CURLOPT_HEADERFUNCTION, write_header_callback))
            FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL, "set header function failed");
        curl_easy_setopt(transfer_info[i].curl_easy_handle, CURLOPT_WRITEHEADER, &transfer_info[i]);
        ...
        // int z=i;
        /* mapping n nodes to one (Obtaining the value of n from environmental variables above)*/
        //*Locks need to be added during multithreading——Preventing conflicts
        transfer_info[i].dataset       = (RV_object_t *)dset[z];
        transfer_info[i].buf           = buf[z];
        transfer_info[i].mem_space_id  = _mem_space_id[z];
        transfer_info[i].file_space_id = _file_space_id[z];
        transfer_info[i].mem_type_id   = mem_type_id[z];
        transfer_info[i].file_type_id  = ((RV_object_t *)dset[z])->u.dataset.dtype_id;
    ...
    for (size_t i = 0; i < count * ServerNodeNum; i++) {
        ...
        // printf("mem_select_npoints: %d , file_select_npoints: %d\n", mem_select_npoints, file_select_npoints);
        ...
        transfer_info[i].curl_headers = curl_slist_append(
            ...
            //************Divide data regions based on the number of nodes**************
            if (i % ServerNodeNum == 0 && transfer_info[i].selection_body) {
                char *curent_data_size = transfer_info[i].selection_body;
                // printf("-> | Default selection_body : %s |\n", transfer_info[i].selection_body);
                Vector *offset = createVector();
                Vector *count  = createVector();
                if (offset == NULL || offset == NULL) {
                    FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL, "Vector alloc failed");
                }
                // Set selection_body
                // 1、get data size
                char *inputCopy                  = strdup(curent_data_size);
                inputCopy                        = inputCopy + 1;
                inputCopy[strlen(inputCopy) - 1] = '\0';
                // printf("-> | inputCopy : %s |\n", inputCopy);

                processInput(inputCopy, offset, count); // alloc data
                // printf("-> | Count->size: %d , data: %d|\n", count->size, count->data[1]);
                // printf("-> | offset->size : %d , data:%d |\n", offset->size, offset->data[1]);

                // 2、Parsing strings [0:1:1,0:2048:1,0:2048:1],保存在
                int i = 0;
                while (i < count->size) {
                    if (count->data[i] >= ServerNodeNum) {
                        // spit
                        flag_Node = true;
                        break;
                    }
                ...
                for (int j = 0; j < count->size; j++) {
                    if (i != j)
                        sum *= count->data[j] - offset->data[j];
                }
                if (flag_Node)
                    splitRangeAndGetInfo(i, ServerNodeNum, sum, node_size, count, offset, Select_Node);
                // free(inputCopy);
                freeVector(count);
                freeVector(offset);
            }
            //*************************************end**********************************
            size_t total_size = 0;
            size_t type_size = H5Tget_size(transfer_info[i].mem_type_id);
            for (int j = 0; j < ServerNodeNum; j++) {
                // printf("ServerNodeNum: %d, j: %d, node_size:%zu \n",ServerNodeNum,j,node_size[j]);
                total_size += node_size[j] * type_size;
            }

            if (ServerNodeNum != 1)
                strcpy(base_URL, URLs_by_Myself[i]);

            if (i % ServerNodeNum == 0) {
                // printf("i:%d , ServerNum :%d", i,ServerNodeNum);
                if ((transfer_info[i].resp_buffer.buffer =
                        (char *)calloc(total_size, sizeof(char))) == NULL)
                    FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, -1,
                                    "failed to allocate memory for response buffer");
            }
            else {
                // 其他迭代分配指向 buffer 的指针
                transfer_info[i].resp_buffer.buffer = transfer_info[i % ServerNodeNum - 1].resp_buffer.buffer +
                                                    node_size[i % ServerNodeNum - 1] * type_size;
            }
            printf("start malloc data, total_size:%u \n", node_size[i % ServerNodeNum]*type_size);
            
            transfer_info[i].resp_buffer.buffer_size  = node_size[i % ServerNodeNum]*type_size;
        ...
        if ((url_len =
                 snprintf(transfer_info[i].request_url, URL_MAX_LENGTH, "%s/datasets/%s/value?h5path=%s%s%s",
                          base_URL, transfer_info[i].dataset->URI, transfer_info[i].dataset->dataset_name,
                          ...
                          is_transfer_binary && transfer_info[i].selection_body &&
                                  (H5S_SEL_POINTS != transfer_info[i].u.read_info.sel_type)
                              ? (ServerNodeNum>1 ? Select_Node[i % ServerNodeNum] : transfer_info[i].selection_body)
                              : "")) < 0)
            //*****mix some bug******
        ...
        if (CURLE_OK !=
            ...
            FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL, "can't set cURL request URL: %s",
                            transfer_info[i].curl_err_buf);
        if (CURLM_OK !=
            curl_multi_add_handle(curl_multi_handle[i % ServerNodeNum], transfer_info[i].curl_easy_handle))
    ...
    //     printf("-> | ServerNode :%s |\n", transfer_info[i].request_url);
    // }
    printf("start buffer data  \n");

    if (ServerNodeNum > 1) {
        curl_global_init(CURL_GLOBAL_ALL);

        //******************Multithreading Start******************
        // pthread_t      threads[ServerNodeNum];
        ...
        for (int i = 0; i < ServerNodeNum; i++) {
            if (CURLM_OK !=
                curl_multi_setopt(curl_multi_handle[i], CURLMOPT_MAX_HOST_CONNECTIONS, NUM_MAX_HOST_CONNS))
                printf("failed to set max concurrent streams for curl multi handle\n");
            // set structural morphology params
            Args_Params *params       = RV_malloc(sizeof(Args_Params));
            params->curl_multi_handle = curl_multi_handle[i];
            params->transfer_info     = transfer_info;
            params->thread_count     = ServerNodeNum;
            params->count             = count;
            params->id                = i;
            if (threadpool_add(pool, RV_curl_multi_perform_multithreading, (void *)params)!=0)
                     printf("failed to create multithreading\n");
            // if (pthread_create(&threads[i], &thread_attr, RV_curl_multi_perform_multithreading,
            //                    (void *)params) != 0)
            //     printf("failed to create multithreading\n");
        }
        ...
        // get  return
        // for (int i = 0; i < ServerNodeNum; ++i) {
        //     printf("-> | actify thread %d|\n", i);
        //     // actify
        //     if (pthread_join(pool->threads[i], NULL) != 0)
        //         printf("failed to perform dataset write\n");
        // }
        pthread_mutex_lock(&pool->lock);
        while (tasks_completed <ServerNodeNum ) {
        ...
        //******************Merge Data of resp_buffer******************
        // referene function rv_dataset_read_cb to merge data and
        printf("-> | Start Merge Data|\n");
        for (int i = 0; i < count; i++) {
            int handle_index = i * ServerNodeNum;

            size_t total_size = 0;
            // for (int j = i * ServerNodeNum; j < ServerNodeNum * (i + 1); ++j) {
            //     total_size += transfer_info[j].resp_buffer.buffer_size;
            //     printf("-> | responseDataSize:%zu |\n",total_size);
            // }
            // size_t *node_size=transfer_info[handle_index].Node_Size;
            size_t type_size = H5Tget_size(transfer_info[handle_index].mem_type_id);
            for (int j = 0; j < ServerNodeNum; ++j) {
                // printf("j:%d, node_size:%zu \n",j,node_size[j]);
                size_t single_data=node_size[j] * type_size;
                printf("实际传输的数据大小:%zu \n",transfer_info[handle_index+j].resp_buffer.buffer_size);
                total_size += single_data;
                // printf("-> | node_size:%zu , type_size:%zu |\n", node_size[j], type_size);
            }
            
            // Assign the merged_buffer pointer to the first response_buffer
            transfer_info[handle_index].resp_buffer.buffer[total_size]   = '\0';

            transfer_info[handle_index].resp_buffer.buffer_size  = total_size;
            
            transfer_info[handle_index].resp_buffer.curr_buf_ptr = transfer_info[handle_index+ServerNodeNum-1].resp_buffer.curr_buf_ptr;
            // printf("total size: %u \n",total_size*type_size);

            if (rv_dataset_read_cb(
                    transfer_info[handle_index].mem_type_id, transfer_info[handle_index].mem_space_id,
                    transfer_info[handle_index].file_type_id, transfer_info[handle_index].file_space_id,
                    transfer_info[handle_index].buf, transfer_info[handle_index].resp_buffer) < 0)
                printf("failed to post-process data read from dataset\n");
        }
        printf("-> | End Merge Data|\n");

        for (size_t j = 0; j < count * ServerNodeNum; j++) {
            transfer_info[j].curl_easy_handle = NULL;

            if (transfer_info[j].transfer_type == WRITE) {
                RV_free(transfer_info[j].u.write_info.write_body);
                transfer_info[j].u.write_info.write_body = NULL;

                RV_free(transfer_info[j].u.write_info.base64_encoded_values);
                transfer_info[j].u.write_info.base64_encoded_values = NULL;
            }

            RV_free(transfer_info[j].request_url);
            transfer_info[j].request_url = NULL;

            if(j%ServerNodeNum==0){
                RV_free(transfer_info[j].resp_buffer.buffer);
                transfer_info[j].resp_buffer.buffer = NULL;
            }
            /* code */
        }
        // printf("-> | free Data|\n");
        //***************************End Merge Data**************************
        for (int i = 0; i < ServerNodeNum; ++i) {
            if (Select_Node[i] != NULL)
                RV_free(Select_Node[i]);
            RV_free(URLs_by_Myself[i]);
        }
        RV_free(url_string_copy);
        curl_global_cleanup();
    }
    else {
        // if (CURLM_OK !=
        //     curl_multi_setopt(curl_multi_handle[0], CURLMOPT_MAX_HOST_CONNECTIONS, NUM_MAX_HOST_CONNS))
        //     FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
        //                     "failed to set max concurrent streams for curl multi handle");
      
   
        if (RV_curl_multi_perform(curl_multi_handle[0], transfer_info, count, rv_dataset_read_cb) < 0)
            FUNC_GOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL, "failed to perform dataset write");

        // if (rv_dataset_read_cb(
        //             transfer_info[0].mem_type_id, transfer_info[0].mem_space_id,
        //             transfer_info[0].file_type_id, transfer_info[0].file_space_id,
        //             transfer_info[0].buf, transfer_info[0].resp_buffer) < 0)
        //         printf("failed to post-process data read from dataset\n");
    }
    ...
    for (size_t i = 0; i < count * ServerNodeNum; i++) {
        ...
        RV_free(transfer_info[i].Node_Size);
...
// void
// binaryToString(void *data, size_t size)
// {
//     char *str = (char *)malloc(size * 2 + 1); // 每个字节转换为两个字符，再加上字符串结束符
//     unsigned char *byte = (unsigned char *)data;

//     for (size_t i = 0; i < size; i++) {
//         sprintf(&str[i * 2], "%02X", byte[i]); // 将每个字节转换为两个十六进制字符
//     }

//     str[size * 2] = '\0'; // 添加字符串结束符
                 ...
                 hid_t _file_space_id[], hid_t dxpl_id, const void *buf[], void **req)
{
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if ((url_len = snprintf(
                 transfer_info[i].request_url, URL_MAX_LENGTH, "%s/datasets/%s/value%s%s?h5path=%s", base_URL,
                 ...
                 transfer_info[i].dataset->dataset_name)) < 0)
            FUNC_GOTO_ERROR(H5E_DATASET, H5E_SYSERRSTR, FAIL, "snprintf error");
                   ...
                   struct response_buffer resp_buffer)
{
    ...
    if (H5T_NO_CLASS == (dtype_class = H5Tget_class(mem_type_id)))
        FUNC_GOTO_ERROR(H5E_DATATYPE, H5E_BADVALUE, FAIL, "memory datatype is invalid");
    // uint16_t(*data)[100] = (uint16_t(*)[100])resp_buffer.buffer;
    // // 打印数组内容
    // for (int i = 0; i < 10; i++) {
    //     for (int j = 0; j < 100; j++) {
    //         printf("%u ", data[i][j]);
    ...
    if ((H5T_REFERENCE != dtype_class) && (H5T_VLEN != dtype_class) && !is_variable_str) {
        ...
        if (sel_type != H5S_SEL_POINTS) {
            ...
            printf("Server response is JSON\n");
        ...
        if (needs_tconv) {
            ...
            // printf("-> file_select_npoints : %d\n",fiWle_select_npoints);
        ...
        // printf("-> | mem_data_size:%zu", mem_data_size);
    }
    else {
...
{
    herr_t ret_value = SUCCEED;
    printf("h5_dataset_write_repsonse_data:%s", (char *)buf);
    return SUCCEED;
}
