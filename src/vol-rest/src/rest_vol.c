...
/*
 * thread pool and read cache by fsc.
 */
...
/* thread pool and cache initial by fsc. */
static herr_t H5_read_thread_pool_init();
static herr_t H5_cache_init();
...
/*-------------------------------------------------------------------------
 * Function:    H5_read_thread_pool_init
 *
 * Purpose:     TODO
 *
 * Return:      Non-negative on success/Negative on failure
...
H5_read_thread_pool_init(){
    ...
    pool = threadpool_create(ServerNodeNum, ServerNodeNum);
    if (pool == NULL) {
        FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
...
H5_rest_init(hid_t vipl_id)
{
    ...
    /* Initialize ThreadPool by fsc */
    if(H5_read_thread_pool_init()<0){
        FUNC_GOTO_ERROR(H5E_VOL, H5E_CANTINIT, FAIL, "Thread_pool failed to initialize");
    }
    // /* Initialize Cache by fsc */
    // if(H5_cache_init()<0){
    //     FUNC_GOTO_ERROR(H5E_VOL, H5E_CANTINIT, FAIL, "Client Cache failed to initialize");
    // }
...
H5rest_term(void)
{
    ...
    // threadpool_destroy by fsc
    if(threadpool_destroy(pool)!=0){
        FUNC_GOTO_ERROR(H5E_VOL, H5E_CLOSEERROR, FAIL, "threadpool_destroy failed !");
    }
...
H5_rest_curl_write_data_callback_no_global(char *buffer, size_t size, size_t nmemb, void *userp)
{
    ...
    // printf("response data from xrootd , data_size:%d x %d\n",size,nmemb);
    ...
    // printf("curr_buf_ptr:");
    // int (*data2)[6] = (int (*)[6])local_response_buffer->curr_buf_ptr; // 将void*指针转换为int(*)[6]类型的指针
    // // 打印数组内容
    // for (int i = 0; i < 5; i++) {
    //     printf("\t");
    //     for (int j = 0; j < 6; j++) {
...
RV_parse_type(char *HTTP_response, void *callback_data_in, void *callback_data_out)
{
    ...
    // printf("parsed_object_string:%s\n",parsed_object_string);
                       ...
                       void *callback_data_out)
{
    ...
    // printf("--------------------------file open-------------------------\n"); //结构体链表
    //     printf("obj_path: %s\n",obj_path); //结构体链表
    //     printf("curl Headers: %s\n",curl_headers->data); //结构体链表
    //     struct curl_slist *curl_headers_tmp =curl_headers->next;
    //     while (curl_headers_tmp!=NULL)
    //     {
    //         printf("curl Headers: %s\n",curl_headers_tmp->data); //结构体链表
    //         curl_headers_tmp=curl_headers_tmp->next;
    //     }
...
// 解压缩数据
void* decompress_data(const void* compressed_data, size_t compressed_size, size_t original_size, size_t *decompressed_size) {
    // 为解压缩数据分配内存
    ...
    // 执行解压缩
    *decompressed_size = ZSTD_decompress(decompressed_data, original_size, compressed_data, compressed_size);
...
void* decompress_data_zlib(const void* compressed_data, size_t compressed_size, size_t original_size, size_t* decompressed_size) {
    // 为解压缩数据分配内存
    void* decompressed_data = malloc(original_size);
    if (decompressed_data == NULL) {
        fprintf(stderr, "内存分配失败\n");
    ...
    // 执行解压缩
    uLongf dest_size = original_size;  // 解压后数据的大小
    int ret = uncompress((Bytef*)decompressed_data, &dest_size, (const Bytef*)compressed_data, compressed_size);
    if (ret != Z_OK) {
        fprintf(stderr, "解压缩失败: %s\n", ret == Z_MEM_ERROR ? "内存不足" : ret == Z_BUF_ERROR ? "输出缓冲区不足" : "数据格式无效");
        free(decompressed_data);
        return NULL;
    }

    *decompressed_size = (size_t)dest_size;
    return decompressed_data;
}

void* decompress_data_lz4(const void* compressed_data, size_t compressed_size, size_t original_size, size_t* decompressed_size) {
    // 为解压缩数据分配内存
    void* decompressed_data = malloc(original_size);
    if (decompressed_data == NULL) {
        fprintf(stderr, "内存分配失败\n");
    ...
    // 执行解压缩
    int ret = LZ4_decompress_safe((const char*)compressed_data, (char*)decompressed_data, compressed_size, original_size);
    if (ret < 0) {
        fprintf(stderr, "解压缩失败: 错误代码 %d\n", ret);
        free(decompressed_data);
        return NULL;
    }

    *decompressed_size = ret;  // 返回解压后的数据大小
    return decompressed_data;
}
    ...
    do {
        ...
        while ((num_prev_running != num_still_running) &&
            ...
            if (curl_multi_msg && (curl_multi_msg->msg == CURLMSG_DONE)) {
                ...
                // printf("-> |  num_still_running :%d , step 2|\n",num_still_running);
                ...
                if (response_code == 503) {
                    ...
                    char* trimmed = strtrim(transfer_info[handle_index].compression_type);
                    printf("压缩类型:%s \n",trimmed);

                    if (strcmp(trimmed, "zstd") == 0) {
                        printf("采用zstd压缩\n");
                        printf("实际传输的数据大小:%zu \n",transfer_info[handle_index].resp_buffer.buffer_size);
                        printf("压缩后大小:%zu \n",transfer_info[handle_index].compression_length);
                        size_t compressed_size = 0;
                        transfer_info[handle_index].resp_buffer.buffer=decompress_data((void*)transfer_info[handle_index].resp_buffer.buffer,transfer_info[handle_index].compression_length,transfer_info[handle_index].resp_buffer.buffer_size,&compressed_size);
                        printf("解压缩后大小:%zu \n",compressed_size);
                    }else if (strcmp(trimmed, "zlib") == 0) {
                        printf("采用zlib压缩\n");
                        printf("实际传输的数据大小:%zu \n",transfer_info[handle_index].resp_buffer.buffer_size);
                        printf("压缩后大小:%zu \n",transfer_info[handle_index].compression_length);
                        size_t compressed_size = 0;
                        transfer_info[handle_index].resp_buffer.buffer=decompress_data_zlib((void*)transfer_info[handle_index].resp_buffer.buffer,transfer_info[handle_index].compression_length,transfer_info[handle_index].resp_buffer.buffer_size,&compressed_size);
                        printf("解压缩后大小:%zu \n",compressed_size);
                    }else if (strcmp(trimmed, "lz4") == 0){
                        printf("采用zlib压缩\n");
                        printf("实际传输的数据大小:%zu \n",transfer_info[handle_index].resp_buffer.buffer_size);
                        printf("压缩后大小:%zu \n",transfer_info[handle_index].compression_length);
                        size_t compressed_size = 0;
                        transfer_info[handle_index].resp_buffer.buffer=decompress_data_lz4((void*)transfer_info[handle_index].resp_buffer.buffer,transfer_info[handle_index].compression_length,transfer_info[handle_index].resp_buffer.buffer_size,&compressed_size);
                        printf("解压缩后大小:%zu \n",compressed_size);
                    }else{
                        printf("无压缩\n");
                    }
...
void *
RV_curl_multi_perform_multithreading(void *args)
{
    Args_Params *params=(Args_Params*)args;
    CURL *curl_multi_handle=params->curl_multi_handle;
    dataset_transfer_info *transfer_info=params->transfer_info;
    ...
    */
        ...
        while ((num_prev_running != num_still_running) &&
            ...
            if (curl_multi_msg && (curl_multi_msg->msg == CURLMSG_DONE)) {
                ...
                if (response_code == 503) {
                    ...
                    num_finished++;
                    succeed_count++;
                    // printf("Thread %d response successful(200)\n",Thread_ID);
    ...
    RV_free(failed_handles_to_retry);
    free(args);
    printf("Thread %d runed !\n",Thread_ID);
    // pthread_exit(NULL);
    pthread_mutex_lock(&pool->lock);
    tasks_completed++;
    printf("Task %d complete: %d \n",Thread_ID,tasks_completed);
    if (tasks_completed % thread_count == 0) {
