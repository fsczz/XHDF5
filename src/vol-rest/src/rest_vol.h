...
//thread_pool and read cache by fsc 
...
typedef struct dataset_transfer_info {
    ...
    char                   compression_type[CURL_ERROR_SIZE];
...
typedef struct Args_Params{
    CURL * curl_multi_handle;
    dataset_transfer_info   *transfer_info;
    size_t  count;
    size_t  thread_count;
    int id;
}Args_Params;
...
herr_t RV_curl_multi_perform(CURL *curl_multi_ptr, dataset_transfer_info *transfer_info, size_t count,
                             herr_t(success_callback)(hid_t mem_type_id, hid_t mem_space_id,
                                                      ...
                                                      struct response_buffer resp_buffer));
void* RV_curl_multi_perform_multithreading(void *args);
